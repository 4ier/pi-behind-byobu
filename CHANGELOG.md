...
- Options: `--config`, `--tmux-socket`, `--title-prefix`, `--max-length`,
  `--strip-prefix`, `--dry-run`, `--no-refresh`, `--quiet`.
- Tests: unit tests for the format expression, the managed block, config path
  resolution and every command, plus an end-to-end test against a throwaway
  `tmux -L` server that proves a running Pi window is renamed and that later Pi
